...
dat.glm <- cbind(dat_clean, cell_metadata_env_clean[,c(21,22,43,69:85)])
...
## Richness
summary(fit.r <- glm.nb(formula(paste0("richness~",covars,"+offset(log(n_not_na))")), data=dat.glm))
summary(fit.r <- update(fit.r, .~.-waom4k_test_settle08))
summary(fit.r <- update(fit.r, .~.-waom4k_test_flux08))

## total cover
summary(fit.c <- glm.nb(formula(paste0("abund_all~",covars,"+offset(log(n_not_na))")), data=dat.glm))
summary(fit.c <- update(fit.c, .~.-waom4k_seafloortemperature))

## SF-abundance
summary(fit.sf <- glm.nb(formula(paste0("abund_SF~",covars,"+offset(log(n_not_na))")), data=dat.glm))
summary(fit.sf <- update(fit.sf, .~.-waom4k_seafloortemperature))
#summary(fit.sf <- glm.nb(abund_SF~depth+depth2+slope+offset(log(n_not_na)), data=dat.glm))

## mobile counts
summary(fit.m <- glm.nb(formula(paste0("abund_mobile~",covars,"+offset(log(n_not_na))")), data=dat.glm))
