...
for (nm in model_ids) {
  ...
  for (i in seq_along(pred.files)) {
    ...
    if (is.null(all.pred.pa.mean)) {
      ...
      all.pred.ab.mean   <- rbind(all.pred.ab.mean, predY.ab.mean)
      all.pred.ab.median <- rbind(all.pred.ab.median, predY.ab.median)
      all.pred.ab.se     <- rbind(all.pred.ab.se, predY.ab.se)
      all.pred.ab.5      <- rbind(all.pred.ab.5, predY.ab.5)
      all.pred.ab.95     <- rbind(all.pred.ab.95, predY.ab.95)
    }
  ...
  out_dir <- file.path(pred_dir, "assembled", nm)
  ...
  for (j in seq_len(ncol(all.pred.pa.mean))) {
    ...
    pred.ra[[1]][all.sel.ra] <- all.pred.pa.mean[, j]
    pred.ra[[2]][all.sel.ra] <- all.pred.pa.median[, j]
    pred.ra[[3]][all.sel.ra] <- all.pred.pa.se[, j]
    pred.ra[[4]][all.sel.ra] <- all.pred.pa.5[, j]
    pred.ra[[5]][all.sel.ra] <- all.pred.pa.95[, j]
  ...
  names(pred.ra) <- paste0("total_abundance",
                           c("_mean","_median","_se","_5","_95"))
