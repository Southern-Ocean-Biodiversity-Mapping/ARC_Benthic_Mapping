...
## - reading in fitted models (from "a_fit_biodiversity_models...") and environmental data
...
## TO DO THIS FOR 500m RESOLUTION WE NEED TO REWRITE SOME OF IT!!!
...
r2 <- rast(paste0(env.derived,"Circumpolar_EnvData_500m_shelf_bathy_ibcso2_depth.tif"))
# 
## load coastline
...
pred_stack.dat$image_quality_score <- 85
pred_stack.dat$annotated_area <- 25

##############################################

##### richness
load(file=paste0(biodiv.dir,"biodiversity_fit_",res,"_richness.Rdata"))

## slope, tpi and waom have some NAs
sel.nas <- which(is.na(pred_stack.dat[,2])|is.na(pred_stack.dat[,3])|is.na(pred_stack.dat[,41]))
ptm = proc.time() ## 5-10min
if(res=="2km"){
  pred.glmm.r <- predict(fit.glmm.r, pred_stack.dat[-sel.nas,], type="response")#, se.fit=TRUE)
}
if(res=="500m"){
...
save(pred.glmm.r, file=paste0(biodiv.dir,"biodiversity_fit_",res,"_richness_glmm_pred.Rdata"))


sel <- which(!is.na(r$depth[]))

pred.ra.r.glmm <- rast(r$depth)
pred.ra.r.glmm[sel[-sel.nas]] <- pred.glmm.r

plot(pred.ra.r.glmm, xlim=c(-600000,500000), ylim=c(-2200000,-1200000), maxcell=15000000)

writeRaster(pred.ra.r.glmm, file=paste0(bio.dir,"Circumpolar_biodiversity_map_",res,"_richness_glmm_pred.tif"))
