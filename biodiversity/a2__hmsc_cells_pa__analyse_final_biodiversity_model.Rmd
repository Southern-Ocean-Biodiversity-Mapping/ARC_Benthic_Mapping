...
- presence-absence (full spatial and environment-only model)
- total abundance (full spatial and environment-only model)
...
dir2   <- paste0(model_dir, "2_fitting_and_running_models")
...
## model selection
nm <- "npp_and_fam_cafe"   # CHANGE THIS TO SWITCH MODEL SETUP
res <- "2km"
...
source(file.path("..","Hmsc_plotBetaSimple.R"))
source(file.path("..","Hmsc_plotGradientSimple.R"))
```
...
## loading old spatial model and CV
load("/pvol/2_fitting_and_running_models/2km_model_cells_1_pa_chains_4_thin_10_samples_800.Rdata")
spatial.models <- models
load(file.path(dir3,"2km_model_cells_1_pa_chains_4_thin_10_samples_800_5foldcv.Rdata"))
load(file.path(dir3,"2km_model_cells_1_pa_chains_4_thin_10_samples_800_MF.Rdata"))
spatial.MF <- MF
spatial.MF.cv <- MF.cv
spatial.preds.cv <- preds.cv
rm(MF, MF.cv, preds.cv, models)
...
plotBetaSimple(m, post=postBeta, mar=c(8, 30, 2, 1), param="Mean", spSort="reverse", main="Mean response estimate", centerEmphasis = 1.5)
if(pngs){dev.off()}
```
...
#   }
# if(grepl("fam_",nm)){
  focal_prod_var <- "log.flux.mean.cafe"
  ## flux
...
## full CV
plot(spatial.MF[[1]]$RMSE[sel], pch=16, main="Environment-spatial model vs Crossvalidation", ylim=c(0.1,0.5), col="blue", ylab="RMSE")
points(spatial.MF.cv[[1]]$RMSE[sel], pch=16, col="red")
...
## full vs env-only
plot(spatial.MF[[1]]$RMSE[sel], pch=16, main="Environment-only vs Environment-spatial", ylim=c(0.1,0.5), col="blue", ylab="RMSE")
points(MF$mENV$RMSE[sel], pch=16, col="black")
legend("topleft", pch=16, col=c("black","blue"), legend=c(paste0("Env-only    (mean RMSE = ",round(mean(MF$mENV$RMSE),2),")"),paste0("Env-spatial (mean RMSE = ",round(mean(spatial.MF[[1]]$RMSE),2),")")))

## full CV vs env-only CV
plot(spatial.MF.cv[[1]]$RMSE[sel], pch=16, main="Environment-only CV vs Environment-spatial CV", ylim=c(0.1,0.5), col="blue", ylab="RMSE")
points(MF.cv$mENV$RMSE[sel], pch=16, col="black")
legend("topleft", pch=16, col=c("black","blue"), legend=c(paste0("CV Env-only    (mean RMSE = ",round(mean(MF.cv$mENV$RMSE),2),")"), paste0("CV Env-spatial (mean RMSE = ",round(mean(spatial.MF.cv[[1]]$RMSE),2),")")))
if(pngs){dev.off()}
```
...
par(mfrow=c(2,2))
## env-only CV
plot(MF[[1]]$AUC[sel], pch=16, main="Environment-only model vs Crossvalidation", ylim=c(0.1,1), ylab="AUC")
points(MF.cv[[1]]$AUC[sel], pch=16, col="red")
legend("bottomright", pch=16, col=c("black","red"), legend=c(paste0("Env-only (mean AUC = ",round(mean(MF[[1]]$AUC),2),")"), paste0("CV         (mean AUC = ",round(mean(MF.cv[[1]]$AUC),2),")")))

## full CV
plot(spatial.MF[[1]]$AUC[sel], pch=16, main="Environment-spatial model vs Crossvalidation", ylim=c(0.1,1), col="blue", ylab="AUC")
points(spatial.MF.cv[[1]]$AUC[sel], pch=16, col="red")
legend("bottomright", pch=16, col=c("blue","red"), legend=c(paste0("Env-spatial (mean AUC = ",round(mean(spatial.MF[[1]]$AUC),2),")"), paste0("CV            (mean AUC = ",round(mean(spatial.MF.cv[[1]]$AUC),2),")")))
