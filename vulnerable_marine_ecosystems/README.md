...
2. `curate_biigle_report.py`: Cleans the BIIGLE report, removes rows with missing info (e.g., width, area, longitude). Saves a csv file where each row is an image, columns are: survey, image area, coordinates, and the coverage in m2 of each VME morpho-taxa (sum of annotations present in the current image for this label).
